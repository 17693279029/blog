    ...
    >
      <div class="head-text-wrap" ref="titleDom">
        ...
        <p class="head-title-sub ban-bred">
          <span @click="goToPage('/')" class="iconfont icon-shouye"> </span> /
          <slot name="action" />
...
const props = defineProps({
  img: {
    type: String,
    default: () =>
      "https://blog-server.bestplayspace.fun/v1/common/files/preview/img/1691567436630.png",
  },
  title: {
    type: String,
...
const handleMousemove = (event) => {
  ...
  if (!props.disBgAnimate) {
    card.value.style.backgroundPosition =
      mousePositionX * 0.5 +
      "%" +
      " " +
      (currentMousePos.y / wHeight) * 25 +
      "%";
  }

  if (!props.disTextAnimate) {
    titleDom.value.style.transform =
      "translateX(" +
...
.page-head {
  ...
  &:hover {
    text-shadow: 0 3px 20px rgba(#fff, 0.8);
  }
