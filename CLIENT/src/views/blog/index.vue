    ...
    >
      <div class="blog-l-content">
        <section class="blog-l-info">
          <img
            class="info-avatar lazy-image"
            data-src="https://blog-server.bestplayspace.fun:3001/api/v1/files/preview?p=425a82c1dcbc37caa33680de28d2a04.jpg&&mimetype=image/jpeg"
          />
          ...
          <p class="search-box">
            <input
              type="text"
              v-model="state.query.params.title"
              @input="goPage(1)"
              placeholder="🔎"
            />
          </p>
      ...
      <section class="blog-list c-mb-40 c-mt-40">
        <template v-if="state.postList.length">
          ...
          >
            <header
              class="blog-header"
              :style="{ width: !post.cover && '100%' }"
            >
              <h2>{{ post.title }}</h2>
              <p>{{ post.abstract }}</p>
              <div class="blog-tip">
                <span
                  >{{ TimeUtils.formatRelativeTime(post.createdAt) }} 发布</span
                >
                <span
          ...
          <div
            class="pagination"
            v-if="
              state.query.pagination.total > state.query.pagination.pageSize
            "
          >
            <a-pagination
...
const backgroundImg = ref(
  "url(http://localhost:3089/v1/common/files/preview/img/1691571900783.png)"
);
...
function updateBackgroundImage() {
  backgroundImg.value = `url(https://blog-server.bestplayspace.fun:3089/v1/common/files/preview/img/${getImageName(
    currentImageIndex
  )}.jpg)`;
      ...
      .blog-cover {
        ...
        img {
          ...
          box-shadow: 0 2px 5px rgba(0, 0, 25, 0.1),
            0 5px 35px 1px rgba(0, 0, 50, 0.2) inset;
        }
      }
    }
  }
